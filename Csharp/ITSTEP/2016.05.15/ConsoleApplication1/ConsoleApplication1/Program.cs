...
namespace CA
{
    class Program
    {
        static public bool EnterNumber(out int number, string word)
        {
            ...
            bool res = Int32.TryParse(temp, out number);
            if (!res)
            {
        ...
        static public void DoResult(int f, int s, char o)
        {
            switch (o)
            {
                case '+':
        ...
        static void Main()
        {
            ...
            do
            {
                Console.WriteLine("\n {0}. First number = {1}, second number = {2}, operator = {3}", N, first, second, @operator);
                ...
                bool result = Int32.TryParse(ch, out choice);
